package txt.jiantu.create.com.txt.per;
...
import txt.jiantu.create.com.txt.R;
import txt.jiantu.create.com.txt.util.LogUtil;
import txt.jiantu.create.com.txt.util.OnActivityPermissionCallback;
import txt.jiantu.create.com.txt.util.OnPermissionCallback;
import txt.jiantu.create.com.txt.util.Util;
...
public class PermissionHelper implements OnActivityPermissionCallback {
        ...
        if (requestCode == REQUEST_PERMISSIONS && permissions.length > 0 && grantResults.length > 0) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                if (Build.MANUFACTURER.equals(JumpPermissionManagement.MANUFACTURER_XIAOMI)) {
                    //针对小米机型
                    if (!hasSelfPermissionForXiaomi(context, permissions[0])) {
                        permiIntentWord = permissions[0];
                        PermissionDialog.intentApplicationPermi(context, permiIntentWord);
                    } else {
    ...
    private void handleSingle(@NonNull String permissionName) {
        if (permissionExists(permissionName)) {
            if (Build.MANUFACTURER.equals(JumpPermissionManagement.MANUFACTURER_XIAOMI)) {
                if (hasSelfPermissionForXiaomi(context, permissionName)) {
